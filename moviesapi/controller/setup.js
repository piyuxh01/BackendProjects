...
exports.newMovie = async (req,res) =>{
    try{
        const {title,year,runtime,genres,director,actors,plot,posturl} = req.body;
        ...
        const newmovies = await movies.create({
            title,year,runtime,genres,director,actors,plot,posturl
        })
...
exports.deleteMovieById = async (req,res) =>{
    try{
        const {id} = req.body;
        const isValid = await movies.findOne({id});
        if(!isValid){
            return res.status(400).json({
                success:false,
        ...
        const deletemovie = await movies.findOneAndDelete(id);
        return res.status(200).json({
            success:true,
            message:"Movie Deleted successfully",
            deletemovie
